import { create } from 'zustand';
import { storeToken, getToken, removeToken } from './authStorage';

import { Platform } from 'react-native';

import api from './api';

const baseUrl = 'https://reni-3d-app.onrender.com/configuracoes'

const useConfiguracoes = create((set, get) => ({
  materiais: [],
  categorias: [],

  consultaOpcoes: async () => {

    try {
      const response = await api.get(`${baseUrl}/opcoes`);

      console.log("Status da Resposta:", response.status);


      const answer = await response.data;

      set({materiais: answer.materiais, categorias: answer.categorias})


    } catch (error) {
      console.error('Erro ao consultar dashboard:', error);
    }
  },

  

}));

export default useConfiguracoes;